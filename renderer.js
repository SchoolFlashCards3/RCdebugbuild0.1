...
const player = {
  ...
  angle: 0,
  moveSpeed: 2 * (TILE),
  hrotSpeed: 2 * (TILE * 0.75),
  vrotSpeed: 1.5 * (TILE * 0.6)
};
...
textures[1].src = "peakmeter100.jpeg";
textures[2].src = "idol.png";
...
function updatePlayer(dt) {
  ...
  if ([1, 2].includes(map[Math.floor(player.y)]?.[Math.floor(newX)])) player.x = player.x; // collide
  else player.x = newX;

  const newY = player.y + dy;
  if ([1, 2].includes(map[Math.floor(newY)]?.[Math.floor(player.x)])) player.y = player.y; // collide
  else player.y = newY;
}
...
function castRay(angle) {
  ...
  while (!hit && dist < MAX_DEPTH) {
    ...
    const cell = map[Math.floor(hitY)]?.[Math.floor(hitX)];
...
function draw() {
  ...
  for (let i = 0; i < NUM_RAYS; i++) {
    ...
    const cell = map[Math.floor(hitY)]?.[Math.floor(hitX)] || 0;
    const tex = cell === 1 ? textures[1] : cell === 2 ? textures[2] : null;

    if (tex) {
      const texX = ((hitX + hitY) % 1) * tex.width;
      ctx.drawImage(tex, texX, 0, 1, tex.height, x, y, (SCREEN_WIDTH / NUM_RAYS) + 1, wallHeight);
    }
  }
}
